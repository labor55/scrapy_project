# -*- coding: utf-8 -*-

# Define here the models for your scraped items
#
# See documentation in:
# https://doc.scrapy.org/en/latest/topics/items.html

import scrapy


class ScrapyAskciV107Item(scrapy.Item):
    # 根目录id(int类型)	1-999
    root_id = scrapy.Field()

    # 上一类	(int类型)	1-999 + 001-999 + 001-999
    parent_id = scrapy.Field()

    # id，自增(int类型)	1-999 + 001-999 + 001-999 + id++
    id = scrapy.Field()

    # 名称(str)
    indic_name = scrapy.Field()

    # 年：1992(int类型)
    data_year = scrapy.Field()

    # 月：1-12(int类型)
    data_month = scrapy.Field()

    # 日：1-31(int类型)
    data_day = scrapy.Field()

    # 频率(1234 季度 ，5678：年月周日  )(int类型)
    frequency = scrapy.Field()

    # 单位(str)
    unit = scrapy.Field()

    # 数据来源(网站名)(str)
    data_source = scrapy.Field()

    # 全国、省份、市等地区(str)
    region = scrapy.Field()

    # 国家(str)
    country = scrapy.Field()

    # 数据产生时间(datetime类型)
    create_time = scrapy.Field()

    # 数据插入时间（爬取时间）(datetime类型)
    # update_time = scrapy.Field()

    # 数值(double)
    data_value = scrapy.Field()

    # 个人编号(str)
    sign = scrapy.Field()

    # 0:无效  1: 有效(int类型)
    status = scrapy.Field()

    # 0 : 未清洗  1 ： 清洗过(int类型)
    cleaning_status = scrapy.Field()
